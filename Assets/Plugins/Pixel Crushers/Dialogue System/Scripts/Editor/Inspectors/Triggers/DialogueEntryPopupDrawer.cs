        ...
        public void UpdateDialogueEntryList(SerializedProperty conversationProp, SerializedProperty prop)
        {
            ...
            if (conversation != null)
            {
                ...
                for (int i = 0; i < conversation.dialogueEntries.Count; i++)
                {
                    ...
                    #region Edits
                    // only show dialogue entries marked with ShowInMenu
                    bool.TryParse(Field.LookupValue(entry.fields, "ShowInMenu"), out bool result);
                    if (result)
                    {
                        // display entry titles instead of subtitle text
                        idToIndex.Add(entry.id, list.Count);
                        indexToId.Add(list.Count, entry.id);
                        list.Add("[" + entry.id + "] " + ((entry.id == 0) ? "<START>" : entry.Title).Replace("/", "\u2215"));
                    }
                    #endregion
